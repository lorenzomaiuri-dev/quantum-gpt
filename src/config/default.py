...
@dataclass
class GPTConfig:
    batch_size: int = 16          # Number of independent sequences processed in parallel per training step
    block_size: int = 32          # Maximum context length: the number of tokens the model can look back at
    ...
    learning_rate: float = 1e-3   # The step size used by the optimizer for weight updates
    device: str = 'cuda' if torch.cuda.is_available() else 'cpu' # Hardware used for computation (GPU or CPU)
