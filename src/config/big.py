...
class GPTConfig:
    ...
    block_size: int = (
        256  # Maximum context length: the number of tokens the model can look back at
    )
    max_iters: int = 20000  # Total number of training iterations (steps)
    eval_interval: int = 500  # How often (in iterations) to run the evaluation loop
    learning_rate: float = (
        3e-4  # The step size used by the optimizer for weight updates
    )
    device: str = (
        "cuda" if torch.cuda.is_available() and not ForceCpu else "cpu"
    ...
    n_embd: int = 384  # Total dimension of the token embeddings (hidden size)
    n_head: int = 6  # Number of attention heads in the Multi-Head Attention mechanism
    n_layer: int = 6  # Number of Transformer blocks (layers) in the model
    dropout: float = (
        0.2  # Probability of dropping neurons during training to prevent overfitting
    )
